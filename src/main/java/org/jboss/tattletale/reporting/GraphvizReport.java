         ...
         while (it.hasNext())
         {
            ...
            if (archive.getType() == ArchiveTypes.JAR)
            {
               ...
               else
               {
                  ...
                  FileWriter dotfw = 
                     new FileWriter(doutput.getAbsolutePath() + File.separator + archive.getName() + ".dot");
               ...
               else
               {
                  ...
                  FileWriter dotfw = 
                     new FileWriter(doutput.getAbsolutePath() + File.separator + archive.getName() + "-package.dot");
                  BufferedWriter dotw = new BufferedWriter(dotfw, 8192);

                  dotw.write("digraph " + dotName(archive.getName()) + "_package_dependencies {" + Dump.NEW_LINE);
                  dotw.write("  node [shape = box, fontsize=10.0];" + Dump.NEW_LINE);

                  Iterator<Map.Entry<String, SortedSet<String>>> resultIt = 
                     archive.getPackageDependencies().entrySet().iterator();
